...
export const Sidebar: React.FC<SidebarProps> = ({ className }) => {
	...
	return (
		<aside
			className={cn(
				'hidden xl:relative xl:flex flex-col flex-[1_1_30%] 3xl:flex-[1_1_20%] px-12 py-32 2xl:py-56 h-full fixed inset-y-0 left-0 xl:h-[100dvh] xl:w-[12rem] bg-secondary text-secondary-foreground',
				className
			)}
		>
			<div
				className='flex flex-col items-center absolute gap-4 top-32 self-center cursor-pointer'
				onClick={() => router.push('/')}
			>
				...
				{/* <div className='inline-flex items-center'>
					<Link href='https://github.com/dandychux' className='group'>
						<GitHubLogoIcon className='w-6 h-6 mr-2 group-hover:text-accent group-hover:fill-accent' />
					</Link>
				</div> */}
			</div>
			...
			<nav className='flex flex-col justify-center px-10 xl:px-16 2xl:px-20 gap-8 mt-auto'>
				{menu_links.map((link) => (
					<NavItem key={link.text} {...link} className='tracking-widest' />
				))}
			</nav>
		</aside>
	);
