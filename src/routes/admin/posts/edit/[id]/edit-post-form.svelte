<script lang="ts">
	...
	const form = superForm(defaults(zod4(updatePostSchema)), {
		validators: zod4Client(updatePostSchema),
		SPA: true,
		onUpdate: async ({ form }) => {
			if (!form.valid) {
	...
	// Initialize time value from existing post date
	let timeValue = $state(() => {
		if ($formData.date) {
			const date =
				...
				<Popover.Root>
					<Popover.Trigger
						class={cn(
							...
							!$formData.tags?.length && "text-white",
						)}
						role="combobox"
						{...props}
					>
		...
		<Form.Control>
			{#snippet children({ props })}
				<div class="flex items-center space-x-2">
					...
					<Form.Label class="cursor-pointer">Published</Form.Label>
				</div>
			{/snippet}
		</Form.Control>
